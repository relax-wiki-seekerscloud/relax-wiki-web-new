...
@Component({
  selector: 'app-advertisements-video',
  templateUrl: './advertisements-video.component.html',
  styleUrls: ['./advertisements-video.component.scss'],
})
export class AdvertisementsVideoComponent implements OnInit {
  ...
  dataValidated(): boolean {
    ...
    if (this.video.nativeElement.value == '') {
      this.showSnackbar(
        'You have to select an Advertisement Video to continue.'
      );
      return false;
    }
    if (!regVideo.test(this.video.nativeElement.value)) {
      this.showSnackbar('Advertisement Video accepts only .mp4');
      return false;
    }
